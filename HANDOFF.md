...
- Media Bridge V5: mantiene l'unico `reqwest::blocking::Client` condiviso introdotto in V3 e la persistenza Connector solo per `/api/movies/:id/stream`. I grandi Range video forward vengono suddivisi sul tratto Media Bridge -> Connector in segmenti bounded da 8 MiB; al massimo due risposte video remote sono attive contemporaneamente. Il body viene trasferito alla WebView attraverso una coda bounded da 16 blocchi x 256 KiB (4 MiB), sufficiente a tenere un piccolo prefetch senza bufferizzare l'intero Range. Su seek/disconnect il residuo del segmento remoto corrente viene drenato verso `sink()` prima di liberare la lane, così reqwest può restituire la TLS al pool. Poster/cover/music/reading continuano a richiedere `Connection: close`; suffix range e piccoli Range video non vengono segmentati.
...
Validazione reale: V3 ha ripristinato catalogo/cover/playback senza rifiuti per `limite connessioni raggiunto` (conteggio reale: 0), ma il riuso video è rimasto insufficiente: 1321 richieste, 1042 connessioni TLS distinte, 279 richieste riutilizzate e massimo 2 richieste per connessione. V4 (32 MiB) non ha risolto il problema: nel test video-only sono state osservate 1479 richieste, 1173 connessioni TLS distinte e massimo 2 richieste per connessione. V5 aggiunge quindi concurrency bounded (2 lane), segmenti da 8 MiB, prefetch bounded da 4 MiB e drain-on-cancel. Tenere `BAIA_DIRECT_MEDIA_DATA_PLANE=false` finché V5 non dimostra riuso TLS nettamente superiore e playback lungo stabile.
