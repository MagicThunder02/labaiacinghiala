...
- Media Bridge V6: mantiene l'unico `reqwest::blocking::Client` condiviso e la persistenza Connector solo per `/api/movies/:id/stream`. I grandi Range video forward restano segmentati a 8 MiB con coda bounded da 16 blocchi x 256 KiB (4 MiB), ma la politica passa a `latest-seek-wins`: per ogni path video un nuovo long Range incrementa una generazione e rende obsoleto quello precedente. Producer e writer controllano la generazione a ogni blocco e terminano senza drenare il residuo remoto obsoleto, privilegiando banda utile e latenza di seek rispetto alla conservazione di una singola TLS. Piccoli Range/suffix range restano indipendenti. I log V6 espongono solo ID opachi e contatori `bytes_from_connector`, `bytes_to_webview`, `bytes_discarded`, senza URL firmati o path filesystem. Poster/cover/music/reading continuano a richiedere `Connection: close`.
...
Validazione reale: V3 ha ripristinato catalogo/cover/playback senza rifiuti per `limite connessioni raggiunto` (conteggio reale: 0), ma il riuso video è rimasto insufficiente: 1321 richieste, 1042 connessioni TLS distinte, 279 richieste riutilizzate e massimo 2 richieste per connessione. V4 (32 MiB) non ha risolto il problema: nel test video-only sono state osservate 1479 richieste, 1173 connessioni TLS distinte e massimo 2 richieste per connessione. V5 ha invece centrato il keep-alive: test 1 = 887 richieste, 18 TLS distinte, 869 riutilizzate, massimo 200, 0 Broken pipe e 0 rifiuti; test 2 = 968 richieste, 14 TLS distinte, 954 riutilizzate, massimo 200, ancora 0 Broken pipe/rifiuti. È però rimasto buffering ripetuto dopo seek e durante playback prolungato su alcuni film. V6 conserva il successo TLS ma sostituisce le due long-lane concorrenti + drain con `latest-seek-wins` per media e stop immediato dei byte obsoleti. Tenere `BAIA_DIRECT_MEDIA_DATA_PLANE=false` finché V6 non dimostra playback/seek stabili e overfetch contenuto.
