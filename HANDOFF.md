...
- Media Bridge V3: un unico `reqwest::blocking::Client` condiviso tra le route registrate con lo stesso fingerprint; persistenza Connector abilitata solo per `/api/movies/:id/stream`, mentre poster/cover/music/reading richiedono `Connection: close`. La segmentazione da 8 MiB sperimentata nella V2 è stata rimossa dopo regressioni reali su cover/catalogo/playback.
...
Validazione reale V3 richiesta: prima verificare che catalogo e cover siano completi e che nei log Connector non compaiano rifiuti per `limite connessioni raggiunto`; poi misurare nuovamente richieste `/baia/v1/media`, connessioni TLS distinte, `transport_reused` e massimo `request_index_on_connection`. Tenere `BAIA_DIRECT_MEDIA_DATA_PLANE=false` finché questa fase non è stabile.
