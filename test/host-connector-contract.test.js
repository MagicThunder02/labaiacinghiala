...
test('Media Bridge V6 usa latest-seek-wins e prefetch bounded solo per i long Range video', () => {
  ...
  assert.match(mediaBridge, /struct VideoStreamCoordinator/);
  assert.match(mediaBridge, /video_coordinator\.begin\(&route\.path\)/);
  assert.match(mediaBridge, /fn is_current\(&self\) -> bool/);
  assert.match(mediaBridge, /sync_channel::<Vec<u8>>\(VIDEO_PIPE_BUFFER_SLOTS\)/);
  assert.doesNotMatch(mediaBridge, /MAX_CONCURRENT_VIDEO_REQUESTS/);
  assert.doesNotMatch(mediaBridge, /drain_connector_response/);
  assert.match(mediaBridge, /V6 prioritizes the new useful Range over preserving this TLS/);
  assert.match(mediaBridge, /bytes_from_connector/);
  assert.match(mediaBridge, /bytes_to_webview/);
  assert.match(mediaBridge, /bytes_discarded/);
